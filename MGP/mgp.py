...
num_generations = 30 # 迭代代数
population_size = 100 # 单个种群数量
max_depth = 3 # 最大深度
#================合并后超参数================#
cross_rate_m = 0.9 # 交叉率
mutate_rate_m = 0.7 # 变异率
num_mgenerations = 80 # 合并后迭代代数
#===========================================#
...
def symbolic_regression(num_generations,num_mgenerations,population_size,data):
    # 初始化两个敌对种群
    a = []
    b = []
    # 随机生成两个种群
    ...
    for generation in range(num_generations):
        ...
        print(f'Generation:{generation+1} \n\
            Best fitness_a = {best_fitness_a} \
            Best fitness_b = {best_fitness_b}') 
        # 选择新的种群
        new_population_a = []
        new_population_b = []
        # 进化a种群
    ...
    print('-'*100)
    print('Merge!')
    population_m = population_a + population_b 
    for merge_generation in range(num_mgenerations):
        fitnesses_m = [fitness_function(ind,data) for ind in population_m]
        best_fitness_m = min(fitnesses_m)
        a.append(best_fitness_m)
        b.append(best_fitness_m)
        best_expr_m = population_m[fitnesses_m.index(best_fitness_m)]
        print('-'*10)
        print(f'Generation:{num_generations+1+merge_generation} \n\
            Best fitness_m = {best_fitness_m}')
        new_population_m = []
        while len(new_population_m) < population_size*2:
            trump = select(population_m,fitnesses_m)
            harris = select(population_m,fitnesses_m)
            child1,child2 = crossover(trump,harris,cross_rate=cross_rate_m)
            child1,child2 = mutation(child1,mutate_rate=mutate_rate_m),mutation(child2,mutate_rate=mutate_rate_m)
            new_population_m.append(child1)
            new_population_m.append(child2)
        population_m = new_population_m
    plt.title(f"最终适应度{min(a[-1],b[-1])}")
    plt.plot(a,label='a')
    plt.plot(b,label='b')
    plt.show()
    return best_expr_m
...
# 执行符号回归
best_expression = symbolic_regression(num_generations=num_generations, num_mgenerations=num_mgenerations,population_size=population_size, data=data)
